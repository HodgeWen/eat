<template>
  <UTable :columns="columns" api="/food/type/page" ref="tableRef">
    <template #tools>
...
const columns = [{ name: '名称', key: 'name' }]
...
const handleDelete = async (id: number) => {
  await http.delete(`/food/type/${id}`)
...
const submit = async () => {
  ...
  if (title.value === '新增') {
    await http.post('/food/type', body).catch(() => {
      loading.value = false
    })
  } else {
    await http.put(`/food/type/${formData.id}`, body).catch(() => {
      loading.value = false
    })
  }
